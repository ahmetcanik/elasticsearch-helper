...
public class BulkInserter implements Closeable {
	...
	private BulkInserter(RestHighLevelClient client, String index, int size) {
		this.client = client;
		this.index = index;
		this.size = size;
	...
	public static BulkInserter create(RestHighLevelClient client, String index, int size) {
		return new BulkInserter(client, index, size);
	}

	public <T extends ElasticEntity> void insert(T entity) throws IOException {
		// insert to buffer
		IndexRequest request = new IndexRequest(index, "_doc");
		String json = ElasticHelper.deleteId(JsonUtil.getSnakeMapper().writeValueAsString(entity));
